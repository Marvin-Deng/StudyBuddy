...
class StudentView {
    ...
    try {
      ...
      if (!name || !school || !email) {
        return res.status(400).json({
          error: "Name, School, Email and grad year are required fields.",
        });
      }
      const results = await StudentController.createStudent(
        name,
        school,
        email,
        grad_year,
  ...
  static async joinStudyGroup(req, res) {
    try {
      ...
      if (!studentId || !groupId) {
        return res.status(400).json({
          error: "studentId and groupId are required in the request body.",
        });
      } else if (
        !Number.isInteger(studentIdInt) ||
        !Number.isInteger(groupIdInt)
      ) {
        return res
          .status(400)
          .json({ error: "studentId and groupId must be integers." });
      }
      const results = await StudentController.joinStudyGroup(
        studentIdInt,
        groupIdInt
      );
      res.status(200).json(results);
  ...
  static async leaveStudyGroup(req, res) {
    try {
      ...
      const results = await StudentController.leaveStudyGroup(
        studentId,
        groupId
      );
      res.status(200).json(results);
