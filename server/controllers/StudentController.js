...
class StudentController {
  static async createStudent(
    ...
    try {
      const results = await pool.query(
        `INSERT INTO STUDENTS (name, school, email, grad_year, major, phone_number) 
        VALUES ($1, $2, $3, $4, $5, $6);`,
        [name, school, email, grad_year, major, phone_number]
      );
      return results.rows;
    } catch (error) {
  ...
  static async joinStudyGroup(student_id, group_id) {
    try {
      const existsQuery = `
          SELECT EXISTS (
            SELECT 1 FROM student_study_groups 
            WHERE student_id=$1 
            AND group_id=$2
          )`;
      ...
      if (exists.rows[0].exists === true) {
        return {
          success: false,
          message: "This user already belongs to this study group",
        };
      }
      const query = `
